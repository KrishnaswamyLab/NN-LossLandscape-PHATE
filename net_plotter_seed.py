...
def set_weights(net, weights, directions=None, step=None):
    ...
    if directions is None:
        ...
        if len(directions) == 2:
            ...
            dy = directions[1]
            dz = directions[2]
            changes = [d0*step[0] + d1*step[1] + d2*step[2] for (d0, d1, d2) in zip(dx, dy, dz)]
        else:
...
def get_random_weights(weights, seed = 0):
    """
    ...
    return [torch.randn(w.size()).to('cuda:0') for w in weights]


def get_random_states(states, seed = 0):
    """
        Produce a random direction that is a list of random Gaussian tensors
...
def create_random_direction(net, dir_type='weights', ignore='biasbn', norm='filter', seed=0):
    """
    ...
    if dir_type == 'weights':
        ...
        direction = get_random_weights(weights, seed = seed)
        normalize_directions_for_weights(direction, weights, norm, ignore)
    elif dir_type == 'states':
        states = net.state_dict() # a dict of parameters, including BN's running mean/var.
        direction = get_random_states(states, seed = seed)
    ...
    if not args.dir_file:
        ...
        if args.y:
            if args.same_dir:
                ...
                ydirection = create_random_direction(args,net, args.dir_type, args.yignore, args.ynorm)
